...
export default function AppearanceMenu() {
    ...
    return (
        <DropdownMenu open={submenuOpen ? true : undefined} onOpenChange={(open) => !submenuOpen && !open && setSubmenuOpen(false)}>
            ...
            {!submenuOpen ? (
                <DropdownMenuContent className="w-50 h-25 flex flex-col justify-around bg-white dark:bg-[#1e1e1e] text-black dark:text-white rounded-xl">
                    ...
                    <DropdownMenuItem className="hover:bg-[#2a2a2a] cursor-pointer">
                        <h3 className="text-sm text-center font-semibold">Сообщить о проблеме</h3>
                    </DropdownMenuItem>
                </DropdownMenuContent>
            ) : (
                <DropdownMenuContent className="w-80 bg-white dark:bg-[#1e1e1e] border border-white/10 text-black dark:text-white rounded-xl">
                    <div className="flex flex-col gap-3 px-3 py-2">
                        ...
                        <div className="flex bg-[#fafafa] dark:bg-[#101010] rounded-xl justify-around mt-2 cursor-pointer">
                            {[
                                { value: 'light', icon: <BsSun size={20} className="cursor-pointer" /> },
                                { value: 'dark', icon: <BsMoon size={20} className="cursor-pointer" /> },
                                ...
                                <button
                                    ...
                                    className={`flex items-center justify-center py-2 px-8 rounded-xl transition-all duration-300 text-sm 
                                            ${theme === opt.value ?
                                            (opt.value === 'dark' ? 'bg-[#1e1e1e] text-white border-1 border-[#444444]' : 'bg-[#f5f5f5] text-black border-2 border-[#e0e0e0]')
